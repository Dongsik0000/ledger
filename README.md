...
| 한 달의 기준이 1일이라 월급 흐름과 맞지 않았다 | **주기 시작일**(예: 25일)을 정하면 이월·잔액·오늘 쓸 수 있는 돈이 그 주기로 계산되고, 거래 내역·통계도 주기로 볼 수 있습니다 |
...
- 빠른 거래 입력(Enter 로 저장)과 최근 기록, 이번 주기 거래로 바로 이동

**거래 내역**
- 달력 월 · 월급 주기 · 최근 12개월 · 전체 기간으로 조회하며 추가 · 수정 · 삭제
- 조회 결과의 수입 · 지출 합계, 내용·메모 검색, 카테고리 · 결제수단 필터 (조회 조건은 주소에 남아 새로고침해도 유지)
- 작성 중에 창을 닫으면 버릴지 먼저 묻습니다
...
- **카드 할부**: 총액 · 개월 수 · 첫 결제월을 넣으면 매달 카드 결제일에 회차별로 기록하고("노트북 (1/3)"), 마지막 회차 뒤 자동으로 정리됩니다
- 휴일 보정까지 반영한 다음 결제일 표시

**요약 / 통계**
- 달력 월 또는 월급 주기 기준으로 전환
...
- 시작 잔액: 기록을 시작한 날의 잔액부터 이월을 계산 (이전 기록은 통계에만 남김)
- 비밀번호 변경
